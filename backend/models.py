...
class Usuario(db.Model):
    ...
    def to_dict(self):
        ...
        if self.tipo == 'plantonista' and self.plantonista:
            data['ranking'] = self.plantonista.ranking
            data['plantonista_id'] = str(self.plantonista.id)
        return data
...
class Plantonista(db.Model):
    ...
    pontuacao_total = db.Column(db.Numeric(10, 2), default=0)
...
class Pontuacao(db.Model):
    ...
    placa_bairro_foco = db.Column(db.Numeric(5, 2), default=0)
    placa_canoas_poa = db.Column(db.Numeric(5, 2), default=0)
    placa_outros = db.Column(db.Numeric(5, 2), default=0)
    pontos_vendas = db.Column(db.Numeric(10, 2), default=0)
    pontos_agenciamentos = db.Column(db.Numeric(10, 2), default=0)
    pontos_placas = db.Column(db.Numeric(10, 2), default=0)
    pontos_total = db.Column(db.Numeric(10, 2), default=0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Plantao(db.Model):
    ...
    status = db.Column(db.String(20), default='disponivel')  # disponivel, reservado, confirmado, cancelado
...
class Alocacao(db.Model):
    ...
    plantonista_id = db.Column(db.String(36), db.ForeignKey('plantonistas.id', ondelete='CASCADE'))
    status = db.Column(db.String(20), default='pendente')  # pendente, confirmado, cancelado, faltou
    tipo = db.Column(db.String(20), default='escolha')  # escolha, atribuido, troca
